...
export default function MessagesPage() {
  ...
  const handleTyping = async (isTyping: boolean) => {
    if (!selectedConversationId || !can('chat:send')) return;
  ...
  const handleCreateProjectChat = async () => {
    if (!selectedProjectForGroup) return;
    try {
        ...
        <div className="h-[72vh]">
          {selectedConversation ? (
            <ChatBox
              ...
              typingLabel={typingLabel ? `${typingLabel} is typing...` : undefined}
              onSend={handleSend}
              onTyping={handleTyping}
            />
          ) : (
            <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-border bg-card/60 text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {searchResults.slice(0, 8).map((result) => (
              <div key={result.id} className="rounded-xl border border-border/70 p-3 text-sm">
                <p>{result.content}</p>
