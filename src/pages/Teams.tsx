...
export default function Teams() {
  ...
  const memberName = (id: string) => users.find((userItem) => userItem.id === id)?.name || 'Unknown';
          ...
          <Card key={team.id} className="overflow-hidden">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {team.members.length > 0 ? team.members.map((memberId) => (
                    ...
                    <span className="text-sm text-muted-foreground">No members added.</span>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {team.projects.length > 0 ? team.projects.map((projectId) => (
                    <Badge key={projectId} variant="secondary" className="text-[10px] gap-1">
...
function TeamForm({
  ...
  const toggleItem = (value: string, selected: string[], setSelected: React.Dispatch<React.SetStateAction<string[]>>) => {
          ...
          <div className="flex flex-wrap gap-2">
            {users.map((teamUser) => (
              <Button
                key={teamUser.id}
                type="button"
                variant={members.includes(teamUser.id) ? 'default' : 'outline'}
                size="sm"
                onClick={() => toggleItem(teamUser.id, members, setMembers)}
              >
                {teamUser.name}
              </Button>
            ))}
          ...
          <div className="flex flex-wrap gap-2">
            {projects.map((project) => (
              <Button
                key={project.id}
                type="button"
                variant={linkedProjects.includes(project.id) ? 'default' : 'outline'}
                size="sm"
                onClick={() => toggleItem(project.id, linkedProjects, setLinkedProjects)}
              >
                {project.title}
              </Button>
            ))}
