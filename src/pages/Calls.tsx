...
import { CallSession, CallType, Conversation } from '@/types';
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-3 md:flex-row md:items-center">
          ...
          {!can('calls:initiate') && (
            <Badge variant="outline" className="rounded-full">Your role can join only</Badge>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-3">
          {activeCalls.map((call) => (
            <button
              ...
              className="w-full rounded-xl border border-border/70 p-3 text-left hover:bg-muted/30"
            >
      ...
      <CallModal
        open={!!selectedCall}
        call={selectedCall}
        canJoin={can('calls:join') && selectedCall?.participants.includes(user!.id) === true}
        onJoin={joinSelectedCall}
        onEnd={endSelectedCall}
      />
    </div>
  );
}
